...
const App: React.FC = () => {
  ...
  const handleLoginSuccess = () => {
    // Note: onAuthStateChanged will also set isAuthenticated(true), but we handle pending actions here
    setIsAuthenticated(true);
    if (pendingAction) {
  ...
  const handleDownloadAudioOnly = () => {
      ...
      a.href = url;
      const ext = result.audioBlob.type.includes('wav') ? 'wav' : 'webm';
      a.download = `recovered_recording.${ext}`;
      a.click();
  };

  const getGmailLink = () => {
    const subject = encodeURIComponent(`Minutes: ${metadata.title}`);
      ...
      <header className="sticky top-0 z-50 glass-panel border-b border-slate-200 dark:border-white/5 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center space-x-3 cursor-pointer group" onClick={handleHome}>
             <div className="w-28 md:w-40 h-10 flex items-center">
                 <img 
                    src="./logo.png" 
                    alt="Zayna" 
                    ...
                    onError={(e) => {
                        ...
                        e.currentTarget.parentElement!.innerHTML = '<span class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Zayna</span>';
                    }} 
                 />
                 <img 
                    src="./logo.png" 
            ...
            <div className="flex items-center justify-between mb-8">
              ...
              {metadata.source === 'screen' && (
                 <div className="flex gap-2">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold border ${metadata.platform === 'zoom' ? 'bg-blue-100 dark:bg-blue-500/20 border-blue-500 text-blue-600 dark:text-blue-400' : 'bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-500'}`}>Zoom</span>
                    <span className={`px-3 py-1 rounded-full text-xs font-bold border ${metadata.platform === 'teams' ? 'bg-indigo-100 dark:bg-indigo-500/20 border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-500'}`}>Teams</span>
                 </div>
              )}
            </div>
            
            <div className="space-y-6">
              {/* Meeting Link Inputs for Screen Mode */}
              {metadata.source === 'screen' && (
        ...
        {status === AppStatus.COMPLETED && (
          <div className="max-w-5xl mx-auto animate-fade-in space-y-8 pb-24 mt-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 glass-panel p-8 md:p-12 rounded-2xl border border-slate-200 dark:border-slate-700 min-h-[60vh] bg-white text-slate-900 shadow-xl">
                ...
                <div className="glass-panel p-6 rounded-2xl border border-indigo-200 dark:border-indigo-500/30 bg-slate-50 dark:bg-slate-900/50 flex flex-col">
                    ...
                    <div className="flex-grow bg-white dark:bg-slate-950 p-4 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 text-sm font-mono whitespace-pre-wrap overflow-auto max-h-[500px]">
                        {result.emailDraft}
                    </div>
                </div>
            </div>
